...
# -- 1) Veri Girişi (Düzeltilmiş 7. haftada en büyük mesafe) --
data = [
    # Hafta 1 (3 gün)
    ...
    # Hafta 7 (5 gün; en yüksek toplam mesafe)
    ...
    {"Day":31,  "Date": "2023-12-27", "Distance_km": 8.5, "Calorie": 663, "Weight_kg": np.nan},  # 8.5 ekledik
...
# (Day-1)//5 + 1 => 5 günlük periyotları haftaya çevir
...
# İstersen "önceki_kilo - bu_hafta_kilo" gibi negatif bir değer istersen
# weekly_sum['WeightLoss_kg'] = weekly_sum['Weight_kg'].shift(1) - weekly_sum['Weight_kg']

# ----- GRAFİK AYARLARI -----
sns.set(style="whitegrid")
plt.rcParams['figure.facecolor'] = '#2b395c'   # Koyu lacivert arka plan
...
# 1) Date vs. Daily Distance (Line Plot)
...
# 2) Date vs. Daily Calorie (Bar Plot => fark yaratmak için)
plt.figure(figsize=(10,4))
...
# 3) Weekly Total Distance vs. Weekly Weight Loss (Bar + Line)
fig, ax1 = plt.subplots(figsize=(10,4))
ax1.set_facecolor('#2b395c')

# Toplam mesafeyi bar grafiği
...
# 4) Weekly Total Calorie vs. Weekly Weight Loss (Line + Line => fark yaratmak için)
plt.figure(figsize=(10,4))
plt.gca().set_facecolor('#2b395c')

# Birinci eksen: Toplam kalori
...
# 5) Date vs. Weekly Weight (Fridays only)
...
# 6) Daily Distance vs. Daily Weight (Scatter + Regression)
# Yalnızca Kilo dolu günleri (Cuma) alıyoruz
...
# 7) Daily Calorie vs. Daily Weight (Scatter + Regression)
plt.figure(figsize=(6,5))
plt.gca().set_facecolor('#2b395c')
